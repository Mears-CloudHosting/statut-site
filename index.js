...
client.on('ready', async () => {
    ...
    try {
        ...
        var online = '<a:online:1180847077463441468>';
        var offline = '<a:offline:1180847059042058372>';
...
setInterval(() => {
              ...
              tcpp.ping({ address: site.addr, port: site.port }, function (err, data) {
                  ...
                  if (previousConnections[site.addr] !== available) {
                    if (available) {
                        ...
                        const availableConnectAlert = `<a:moneybot:1180846469842993204> Le site: \`\`${site.addr}\`\` est __redisponible__ à: **${now}** <a:coeur2:1180846405431066654>`;
                        logChannel.send(availableConnectAlert);
                    } else {
                        // Site a perdu sa connexion, envoyer une alerte
                        const now = new Date().toLocaleString('fr-FR', { timeZone: "Europe/Paris" });
                        const unavailableConnectAlert = `<a:mmhh2:1180846449181851748> | Le site \`\`${site.addr}\`\` est __down,__ détecté à: **${now}** <a:alerte:1180846386330210374>`;
  ...
  Promise.all(pingFields).then(fields => {
    const pingEmbed = new EmbedBuilder()
        ...
        .setURL("https://stats.uptimerobot.com")
        .setColor('#4285F4')
        .addFields(...fields)
        ...
        .setColor('#4285F4')
        .setTitle("Acces au panel")
        .setURL("https://panel.cloud-hebergeur.fr")
        .setDescription("*Clique sûr le titre pour acceder au panel*")
